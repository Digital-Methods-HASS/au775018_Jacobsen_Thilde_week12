...
date: 'Week 12, Thilde Laue Jacobsen'
...
Example: Just want to get text from a single page (e.g. Page 70)? 
```{r single-page}
got_p70 <- got_text[70]
got_p70
```

See how that compares to the text in the PDF on Page 70. What has pdftools added and where?
...
got_df <- data.frame(got_text) %>% 
  mutate(full_text = str_split(got_text, pattern = '\n')) %>% 
  unnest(full_text) %>% 
...
ggplot(data = got_top100, aes(label = word, size = n)) +
  ...
  scale_color_gradientn(colors = c("red","blue","purple")) +
  theme_minimal()
```
...
Look back at the GOT report, and search for "Lord." Is it typically associated with positive, or something else? 
...
Wait, so "lord" is showing up in NRC lexicon as "positive" and "negative"? Let's check:
```{r nrc-confidence}
